...
for compound in ["SOFT", "MEDIUM", "HARD"]:
    defaults = default_covariates(df, model, circuit="Sakhir", compound=compound)
    print(f"{compound} defaults:", defaults)
    covariates = dict(defaults)
    covariates["track_temp_c"] = 35.0
    query = pd.DataFrame({
        "tire_compound": [compound]*len(ages), "circuit": ["Sakhir"]*len(ages),
        "tire_age_laps": ages, **{k: [v]*len(ages) for k, v in covariates.items()},
    })
    preds = model.predict(query)
    print(f"{compound} predictions: ", np.round(preds, 3))
    print("")
