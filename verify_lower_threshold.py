...
for circuit, threshold in [("Sakhir", 1.0), ("Monaco", 0.3), ("Silverstone", 0.3)]:
    defaults = default_covariates(df, model, circuit=circuit)
    report = generate_tire_report(
        model, circuit=circuit, track_temp_c=35.0, degradation_threshold_s=threshold, extra_covariates=defaults,
    )
